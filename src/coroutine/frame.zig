...
fn smallLayout() FrameLayout {
    return .{
        .total_size = 64,
        .param_region = .{ .start = 0, .size = 16 },
        .local_region = .{ .start = 16, .size = 32 },
        .temp_region = .{ .start = 48, .size = 16 },
    };
}
...
test "CoroutineFrame.initFixed 设置初始状态" {
    ...
    try testing.expect(frame.locals_offset >= @sizeOf(CoroutineFrame));
    try testing.expectEqual(@as(u32, 64), frame.locals_size);
}
...
test "CoroutineFrame 状态转换" {
    ...
    try testing.expectEqual(CoroutineStatus.running, frame.getStatus());
...
test "CoroutineFrame panic 缓冲区读写" {
    ...
    // 超长消息截断
    const long_msg = "a" ** 200;
    frame.setPanic(long_msg);
    try testing.expectEqual(@as(usize, CoroutineFrame.PANIC_BUF_SIZE), frame.getPanic().len);
}

test "FramePool 分配与释放" {
    ...
    try testing.expect(stats.free_count == 1);
    try testing.expect(stats.active_frames == 0);
}

test "FramePool free list 复用" {
    var threaded = std.Io.Threaded.init(testing.allocator, .{});
    defer threaded.deinit();
    const io = threaded.io();
    ...
    pool.free(frame2);
    const stats = pool.getStats();
    try testing.expect(stats.backing_alloc_count == 1); // 只向 backing 申请过一次
}

test "FramePool bucketFor size class 选择" {
    try testing.expectEqual(@as(usize, 0), FramePool.bucketFor(32));
    try testing.expectEqual(@as(usize, 0), FramePool.bucketFor(64));
    try testing.expectEqual(@as(usize, 1), FramePool.bucketFor(65));
    try testing.expectEqual(@as(usize, 1), FramePool.bucketFor(128));
    try testing.expectEqual(@as(usize, 2), FramePool.bucketFor(256));
    try testing.expectEqual(@as(usize, 5), FramePool.bucketFor(8192));
}
