...
impl<'a> Parser<'a> {
    ...
    fn alloc_expr(&self, span: Span, expr: Expr<'a>) -> ExprRef<'a> {
        &*self.arena.alloc(Spanned { span, node: expr })
    }

    fn alloc_stmt(&self, span: Span, stmt: Stmt<'a>) -> StmtRef<'a> {
        &*self.arena.alloc(Spanned { span, node: stmt })
    }

    fn alloc_type(&self, span: Span, ty: TypeNode<'a>) -> TypeRef<'a> {
        &*self.arena.alloc(Spanned { span, node: ty })
    }

    fn alloc_pattern(&self, span: Span, pat: Pattern<'a>) -> PatternRef<'a> {
        &*self.arena.alloc(Spanned { span, node: pat })
    }
        ...
        while !self.is_at_end() {
            ...
            if self.match_token(TokenKind::Eq) {
                ...
                declarations.push(self.spanned_decl(
                    expr.span,
                    Decl::ExprDecl {
                        expr: self.alloc_expr(expr.span, Expr::VoidLit),
                        stmt: Some(stmt),
                    },
                ));
            } else {
    ...
    fn try_parse_decl(&mut self) -> Option<Spanned<Decl<'a>>> {
        ...
        if self.check(TokenKind::KwFun) {
            return self.parse_fun_decl(visibility, is_async).ok();
        }
    ...
    fn parse_fun_decl(&mut self, visibility: Visibility, is_async: bool) -> ParseResult<Spanned<Decl<'a>>> {
    ...
    /// 类型解析入口：处理前缀 &T / *T
    fn parse_type(&mut self) -> ParseResult<TypeRef<'a>> {
    ...
    fn parse_function_type(&mut self) -> ParseResult<TypeRef<'a>> {
    ...
    /// 解析可空类型：T?（链式）
    fn parse_nullable_type(&mut self) -> ParseResult<TypeRef<'a>> {
        let mut ty = self.parse_primary_type()?;
        while self.match_token(TokenKind::Question) {
            ty = self.alloc_type(ty.span, TypeNode::Nullable { inner: ty });
        }
        Ok(ty)
    }

    /// 解析基本类型：命名/泛型，后缀数组 [N]
    fn parse_primary_type(&mut self) -> ParseResult<TypeRef<'a>> {
        if self.check(TokenKind::LParen) {
    ...
    /// 解析记录类型：(field: Type, ...)
    fn parse_record_type(&mut self) -> ParseResult<TypeRef<'a>> {
    ...
    /// 表达式解析入口
    pub fn parse_expr(&mut self) -> ParseResult<ExprRef<'a>> {
        self.parse_binary(MIN_PREC)
    }

    /// 单一 Pratt 解析器
    fn parse_binary(&mut self, min_prec: u8) -> ParseResult<ExprRef<'a>> {
    ...
    /// 解析后缀运算
    fn parse_postfix(&mut self) -> ParseResult<ExprRef<'a>> {
        let mut expr = self.parse_primary()?;
        loop {
            if self.match_token(TokenKind::Question) {
                let op_tok = self.previous();
                // type_cast 后紧跟 ? → 安全转换：重新分配一个 safe=true 的 TypeCast 节点
                if let Expr::TypeCast { target, expr: inner, safe: false } = &expr.node {
    ...
    /// 解析调用参数（已在 `(` 处）
    fn parse_call_args(&mut self) -> ParseResult<(Vec<ExprRef<'a>>, Option<Vec<TypeRef<'a>>>)> {
        // 可选 turbofish <T> 在 ( 之前
    ...
    /// 解析整数字面量，分离数字部分与类型后缀
    fn parse_int_literal(&self, tok: Token<'a>) -> ExprRef<'a> {
        let raw = tok.lexeme;
        let mut i: usize = 0;
    ...
    /// 解析负整数字面量，将负号合并进 raw
    fn parse_negative_int_literal(&self, lit_tok: Token<'a>) -> ParseResult<ExprRef<'a>> {
    ...
    /// 解析浮点字面量，分离数字部分与类型后缀
    fn parse_float_literal(&self, tok: Token<'a>) -> ExprRef<'a> {
        let raw = tok.lexeme;
    ...
    /// 解析负浮点字面量
    fn parse_negative_float_literal(&self, lit_tok: Token<'a>) -> ParseResult<ExprRef<'a>> {
        let raw = lit_tok.lexeme;
        let bytes = raw.as_bytes();
        let mut i = raw.len();
        while i > 0 && bytes[i - 1].is_ascii_digit() {
    ...
    /// 解析字符串字面量（含插值处理）
    fn parse_string_literal(&mut self, tok: Token<'a>) -> ParseResult<ExprRef<'a>> {
        let raw = tok.lexeme;
    ...
    /// 对插值表达式文本进行词法+语法分析
    fn parse_interpolation_expr(&mut self, text: &'a str) -> ParseResult<ExprRef<'a>> {
    ...
    fn parse_lambda_fun(&mut self, is_async: bool) -> ParseResult<ExprRef<'a>> {
        let fun_tok = self.advance(); // 'fun'
    ...
    fn try_parse_lambda(&mut self, saved: usize, span: Span) -> Option<ExprRef<'a>> {
        let saved_error_count = self.errors.len();
        let mut params = Vec::new();
        if !self.check(TokenKind::RParen) {
            if self.parse_lambda_param_list(&mut params).is_err() {
                self.errors.truncate(saved_error_count);
                self.current = saved;
                return None;
            }
    ...
    fn parse_paren_or_record_or_lambda(&mut self) -> ParseResult<ExprRef<'a>> {
    ...
    fn parse_type_cast(&mut self) -> ParseResult<ExprRef<'a>> {
        let name_tok = self.advance();
        let span = token_span(&name_tok);
    ...
    fn parse_cast_builder(&mut self) -> ParseResult<ExprRef<'a>> {
    ...
    /// 解析 if 表达式
    fn parse_if_expr(&mut self) -> ParseResult<ExprRef<'a>> {
    ...
    /// 解析 match 表达式
    fn parse_match_expr(&mut self) -> ParseResult<ExprRef<'a>> {
    ...
    fn parse_inline_trait_value(&mut self) -> ParseResult<ExprRef<'a>> {
        let trait_tok = self.advance(); // 'trait'
    ...
    /// 解析数组字面量
    fn parse_array_literal(&mut self) -> ParseResult<ExprRef<'a>> {
        let bracket_tok = self.previous();
    ...
    /// 解析块表达式
    fn parse_block_expr(&mut self) -> ParseResult<ExprRef<'a>> {
        ...
        while !self.check(TokenKind::RBrace) && !self.is_at_end() {
            if self.is_stmt_start() {
                let stmt = self.parse_stmt()?;
                if matches!(stmt.node, Stmt::Expression { .. }) && self.check(TokenKind::RBrace) {
    ...
    /// 语句解析入口
    fn parse_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    /// 解析 fun 语句
    fn parse_fun_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_val_decl(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_var_decl(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_return_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_defer_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
        let defer_tok = self.previous();
        let span = token_span(&defer_tok);
        let expr = self.parse_expr()?;
        if self.match_token(TokenKind::Eq) {
            let value = self.parse_expr()?;
            let assign_expr = self.alloc_expr(expr.span, Expr::Assign {
                target: expr,
                value,
            });
    ...
    fn parse_throw_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
        let throw_tok = self.previous();
        let expr = self.parse_expr()?;
        Ok(self.alloc_stmt(token_span(&throw_tok), Stmt::Throw { expr }))
    }

    fn parse_for_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_while_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
    ...
    fn parse_loop_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
        let loop_tok = self.previous();
        let body = self.parse_expr()?;
        Ok(self.alloc_stmt(token_span(&loop_tok), Stmt::Loop { body }))
    }

    fn parse_expr_or_assignment_stmt(&mut self) -> ParseResult<StmtRef<'a>> {
        let expr = self.parse_expr()?;
        if self.match_token(TokenKind::Eq) {
            ...
            match &expr.node {
                Expr::Ident(_) => {
                    return Ok(self.alloc_stmt(expr.span, Stmt::Assignment {
                        target: expr,
                        value,
                    }));
                }
                Expr::FieldAccess { recv, field, .. } => {
                    return Ok(self.alloc_stmt(token_span(&eq_tok), Stmt::FieldAssignment {
                        object: *recv,
                        field: *field,
                        value,
                    }));
                ...
                _ => {
                    return Ok(self.alloc_stmt(token_span(&eq_tok), Stmt::Assignment {
                        target: expr,
                        value,
                    }));
                }
        ...
        Ok(self.alloc_stmt(expr.span, Stmt::Expression { expr }))
    }
    ...
    fn parse_pattern(&mut self) -> ParseResult<PatternRef<'a>> {
        self.parse_or_pattern()
    }

    fn parse_or_pattern(&mut self) -> ParseResult<PatternRef<'a>> {
    ...
    fn parse_primary_pattern(&mut self) -> ParseResult<PatternRef<'a>> {
    ...
    /// 解析构造器模式
    fn parse_constructor_pattern(&mut self, name_tok: Token<'a>) -> ParseResult<PatternRef<'a>> {
        self.advance(); // '('
    ...
    /// 解析记录模式
    fn parse_record_pattern(&mut self) -> ParseResult<PatternRef<'a>> {
...
fn int_to_key<'a>(arena: &'a Bump, idx: usize) -> &'a str {
    let s = idx.to_string();
    arena.alloc_str(&s)
}
...
mod parser_tests {
    ...
    fn test_parse_binary_expr() {
        ...
        let module = parse_module(src, &arena).unwrap();
        match &module.declarations[0].node {
            Decl::FunDecl { body, .. } => match &body.node {
                Expr::Block { stmts, .. } => match &stmts[0].node {
                    Stmt::ValDecl { value, .. } => match &value.node {
                        Expr::Binary {
                            op: BinaryOp::Add,
                            ..
                        } => {}
    ...
    fn test_parse_nested_generics() {
        ...
        match &module.declarations[0].node {
            Decl::TypeDecl { def, .. } => match def {
                TypeDef::Alias { target } => match &target.node {
...
impl Printer {
    ...
    /// 将模块打印为规范文本
    pub fn print_module(&mut self, module: &Module<'_>) -> &str {
    ...
    fn print_decl(&mut self, decl: &Decl<'_>) {
        match decl {
            Decl::FunDecl {
                visibility,
                name,
    ...
    fn print_type_def(&mut self, def: &TypeDef<'_>) {
        match def {
            TypeDef::Adt { constructors } => {
                self.write_line("(adt");
                self.indent();
    ...
    fn print_methods(&mut self, methods: &[MethodDecl<'_>]) {
        if methods.is_empty() {
    ...
    fn print_method(&mut self, m: &MethodDecl<'_>) {
    ...
    fn print_associated_types(&mut self, assoc: &[AssociatedType<'_>]) {
        if assoc.is_empty() {
    ...
    fn print_type_constraints(&mut self, constraints: &[TypeConstraint<'_>]) {
        ...
        for c in constraints {
            ...
            self.indent();
            self.print_type(&c.concrete_type.node);
            self.dedent();
            self.write_line(")");
        }
        self.dedent();
    ...
    fn print_type_params(&mut self, params: &[TypeParam<'_>]) {
        if params.is_empty() {
    ...
    fn print_return_type(&mut self, rt: &Option<TypeRef<'_>>) {
        match rt {
            Some(ty) => {
                self.write_line("(return_type");
                self.indent();
    ...
    fn print_type(&mut self, ty: &TypeNode<'_>) {
        match ty {
            TypeNode::Named { name } => {
            ...
            TypeNode::Array {
                ...
                self.indent();
                self.print_type(&element_type.node);
                self.dedent();
                self.write_line(")");
                match size {
    ...
    fn print_kind(&mut self, kind: &Kind) {
        match kind {
            ...
            Kind::Arrow { param, result } => {
                ...
                self.indent();
                self.print_kind(param);
                self.dedent();
                self.write_line(")");
                self.write_line("(result");
                self.indent();
                self.print_kind(result);
                self.dedent();
                self.write_line(")");
                self.dedent();
                self.write_line(")");
    ...
    fn print_expr(&mut self, expr: &Expr<'_>) {
        match expr {
            ...
            Expr::Binary { op, lhs, rhs } => {
                ...
                self.write_line("(lhs");
                self.indent();
                self.print_expr(&lhs.node);
                self.dedent();
                self.write_line(")");
                self.write_line("(rhs");
                self.indent();
                self.print_expr(&rhs.node);
                self.dedent();
                self.write_line(")");
                self.dedent();
                self.write_line(")");
            ...
            Expr::If {
                ...
                self.indent();
                self.print_expr(&then_branch.node);
                self.dedent();
                self.write_line(")");
                match else_branch {
                    Some(e) => {
            ...
            Expr::Match { scrutinee, arms } => {
                ...
                self.indent();
                self.print_expr(&scrutinee.node);
                self.dedent();
                self.write_line(")");
                if arms.is_empty() {
                    ...
                    for arm in arms {
                        match arm {
                            SelectArm::Receive {
                                ...
                                self.indent();
                                self.print_expr(&channel_expr.node);
                                self.dedent();
                                self.write_line(")");
                                match binding {
                                    Some(name) => {
    ...
    fn print_stmt(&mut self, stmt: &Stmt<'_>) {
        match stmt {
            Stmt::ValDecl {
                name,
                type_annotation,
    ...
    fn print_pattern(&mut self, pat: &Pattern<'_>) {
        match pat {
            ...
            Pattern::Record { fields } => {
                ...
                if fields.is_empty() {
                    ...
                    for f in fields {
                        self.write_line(&format!("(field \"{}\"", f.name));
                        self.indent();
                        self.print_pattern(&f.pattern.node);
                        self.dedent();
                        self.write_line(")");
                    }
                    self.dedent();
            ...
            Pattern::OrPattern { left, right } => {
                ...
                self.indent();
                self.print_pattern(&left.node);
                self.dedent();
                self.write_line(")");
                self.write_line("(right");
                self.indent();
                self.print_pattern(&right.node);
                self.dedent();
                self.write_line(")");
                self.dedent();
                self.write_line(")");
            ...
            Pattern::Guard { pattern, condition } => {
                ...
                self.write_line("(pattern");
                self.indent();
                self.print_pattern(&pattern.node);
                self.dedent();
                self.write_line(")");
...
/// 便捷入口：将模块打印为规范文本
pub fn print_module(module: &Module<'_>) -> String {
    let mut p = Printer::new();
    p.print_module(module).to_string()
}

/// 便捷入口：将声明打印为规范文本
pub fn print_decl(decl: &Spanned<Decl<'_>>) -> String {
    let mut p = Printer::new();
    p.print_decl(&decl.node);
    p.buf
}
