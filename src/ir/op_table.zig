...
test "op_table: completeness — every NodeOp variant has an entry" {
    inline for (std.meta.fields(NodeOp)) |field| {
        const op: NodeOp = @enumFromInt(field.value);
        const e = entry(op);
        try std.testing.expect(e.op == op);
        try std.testing.expect(e.name.len > 0);
    }
}

test "op_table: isScalar consistency with node.zig" {
    try std.testing.expect(isScalar(.int_add));
    try std.testing.expect(isScalar(.const_i));
    try std.testing.expect(!isScalar(.vec_map));
    try std.testing.expect(!isScalar(.halt_return));
}

test "op_table: isVector / isHalt consistency" {
    try std.testing.expect(isVector(.vec_map));
    try std.testing.expect(isVector(.vec_sink));
    try std.testing.expect(!isVector(.int_add));
    try std.testing.expect(isHalt(.halt_return));
    try std.testing.expect(isHalt(.halt_continue));
    try std.testing.expect(!isHalt(.call));
}

test "op_table: has_nested covers subgraph-holding ops" {
    try std.testing.expect(hasNested(.vec_map));
    try std.testing.expect(hasNested(.vec_fold));
    try std.testing.expect(hasNested(.scalar_loop));
    try std.testing.expect(hasNested(.closure_make));
    try std.testing.expect(hasNested(.route_dispatch));
    try std.testing.expect(hasNested(.cleanup_register));
    try std.testing.expect(!hasNested(.vec_source));
    try std.testing.expect(!hasNested(.call));
}

test "op_table: batch mappings" {
    try std.testing.expect(batchBinOp(.int_add).? == .add);
    try std.testing.expect(batchBinOp(.float_mul).? == .mul);
    try std.testing.expect(batchBinOp(.int_and).? == .band);
    try std.testing.expect(batchBinOp(.int_shl) == null);
    try std.testing.expect(batchBinOp(.cmp_eq) == null);
    try std.testing.expect(batchUnaryOp(.int_neg).? == .neg);
    try std.testing.expect(batchUnaryOp(.float_abs).? == .abs);
    try std.testing.expect(batchUnaryOp(.int_not).? == .bnot);
    try std.testing.expect(batchUnaryOp(.int_add) == null);
}

test "op_table: foldable covers const + foldable binary ops" {
    // 常量
    try std.testing.expect(foldable(.const_i));
    try std.testing.expect(foldable(.const_bool));
    // 可折叠二元 op
    try std.testing.expect(foldable(.int_add));
    try std.testing.expect(foldable(.cmp_eq));
    try std.testing.expect(foldable(.bool_and));
    // 不折叠
    try std.testing.expect(!foldable(.int_shl));
    try std.testing.expect(!foldable(.float_add));
    try std.testing.expect(!foldable(.int_neg));
    try std.testing.expect(!foldable(.bool_not));
    try std.testing.expect(!foldable(.cast));
    try std.testing.expect(!foldable(.vec_map));
}
