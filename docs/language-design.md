...
**参数化条件实现（conditional impl）**：`impl` 头部的类型实参可含自由类型变量，
配合末尾 `with` 子句声明这些变量须满足的 Trait——语法与函数 bound 同构。读作
"当 `T` 满足 `Show` 时，`Box<T>` 也满足 `Show`"：
...
// 多个类型参数 / 多个约束：逗号分隔的 bound 列表
impl Show<Pair<A, B>> with Show<A>, Show<B> {
    fun show(self) : str { "(" + self.first.show() + ", " + self.second.show() + ")" }
}
```

头部里未声明的名字（`T`、`A`、`B`）即被量化的自由类型变量。Orphan 规则（§2.7.6）
照常适用：条件 impl 须写在拥有 Trait 或目标类型的模块中。
...
`@embedFile` 编进解释器二进制。`use <Name>` 加载模块时，**项目内同名文件优先**（允许用户
...
- **`List`** — 函数式链表 `List<T> = | Nil | Cons(T, List<T>)`，配套
  ...
  等算子。`pub type` 的构造器（`Nil`/`Cons`）随 `use List` 一并导入到值命名空间。
  本模块还用参数化条件实现（§2.7.3）提供 `impl Show<List<T>> with Show<T>`，
  故元素可 `Show` 的链表自身也可 `show()`。
- **`Compare`** — 类型类 trait `Eq` / `Ord` / `Show`，及对内建类型的实现：
  `Show`/`Eq`/`Ord` 覆盖 `i64`（所有整数宽度）、`f64`（所有浮点宽度）、`str`、`char`，
  `Show`/`Eq` 另覆盖 `bool`。容器类型可借参数化条件实现（§2.7.3）扩展，如 `List` 模块对 `List<T>` 的 `Show`。
