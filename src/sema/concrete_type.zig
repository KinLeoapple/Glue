...
test "ConcreteType.typeName" {
    ...
    try std.testing.expectEqualStrings("i64", i64_ty.typeName().?);

    const adt_ty = ConcreteType{ .adt_type = .{ .name = "Option", .type_args = &.{} } };
    try std.testing.expectEqualStrings("Option", adt_ty.typeName().?);
}

test "unify 标量相同" {
    var a = ConcreteType{ .i64_type = {} };
    var b = ConcreteType{ .i64_type = {} };
    try unify(&a, &b);
}

test "unify 标量不同失败" {
    var a = ConcreteType{ .i64_type = {} };
    var b = ConcreteType{ .f64_type = {} };
    try std.testing.expectError(error.TypeMismatch, unify(&a, &b));
}

test "unify type_var 绑定" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const tv = try a.create(TypeVar);
    tv.* = .{ .id = 0, .bound = null };
    const var_ty = try a.create(ConcreteType);
    var_ty.* = .{ .type_var = tv };

    const i64_ty = try a.create(ConcreteType);
    i64_ty.* = .{ .i64_type = {} };

    try unify(var_ty, i64_ty);
    try std.testing.expect(tv.bound == i64_ty);
}

test "unify never_type 与任意类型" {
    var a = ConcreteType{ .never_type = {} };
    var b = ConcreteType{ .i64_type = {} };
    try unify(&a, &b);
    try std.testing.expect(a == .i64_type);
}

test "unify unknown_type 与任意类型" {
    var a = ConcreteType{ .unknown_type = {} };
    var b = ConcreteType{ .i64_type = {} };
    try unify(&a, &b);
    try std.testing.expect(a == .i64_type);
}

test "unify occurs check 失败" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();
    ...
    inner.* = .{ .type_var = tv };
    const list_ty = try a.create(ConcreteType);
    list_ty.* = .{ .array_type = .{ .element_type = inner, .size = null } };

    try std.testing.expectError(error.OccursCheckFailed, unify(var_ty, list_ty));
}
...
test "resolve 解析多层绑定" {
    ...
    const tv1 = try a.create(TypeVar);
    tv1.* = .{ .id = 0, .bound = null };
    const tv2 = try a.create(TypeVar);
    tv2.* = .{ .id = 1, .bound = null };
    const ty1 = try a.create(ConcreteType);
    ty1.* = .{ .type_var = tv1 };
    const ty2 = try a.create(ConcreteType);
    ty2.* = .{ .type_var = tv2 };
    const i64_ty = try a.create(ConcreteType);
    i64_ty.* = .{ .i64_type = {} };
    ...
    try std.testing.expect(resolve(ty1) == i64_ty);
}

test "ConcreteEnv lookup 父作用域" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    var parent = ConcreteEnv.init(a);
    defer parent.deinit();
    const ty = try a.create(ConcreteType);
    ty.* = .{ .i64_type = {} };
    try parent.define("x", ty);

    const child = try parent.createChild();
    defer child.deinit();
    try std.testing.expect(child.lookup("x") == ty);
    try std.testing.expect(child.lookup("y") == null);
}

test "TypeAllocator.freshTypeVar 创建独立 id" {
    var alloc = TypeAllocator.init(std.testing.allocator);
    defer alloc.deinit();

    const tv1 = try alloc.freshTypeVar();
    const tv2 = try alloc.freshTypeVar();
    try std.testing.expect(tv1.type_var.id == 0);
    try std.testing.expect(tv2.type_var.id == 1);
    try std.testing.expect(tv1 != tv2);
}

test "TypeAllocator.freshRigidVar 不可与不同类型统一" {
    var alloc = TypeAllocator.init(std.testing.allocator);
    defer alloc.deinit();

    const rigid = try alloc.freshRigidVar();
    const i64_ty = try alloc.make(.{ .i64_type = {} });
