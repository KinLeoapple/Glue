...
**M5k — nullary 方法 / 构造器作值 / 冲突防护（覆盖维持 30）✅ 已完成**
攻 phase2 实现 4 个真实特性，但 phase2 最终因 trait 冲突消解仍回退。净收益是 4 个稳健特性 + 冲突防护：
- **nullary trait 方法**：`fun zero()`（无 self 无参）作自由函数调用 `zero()`——无 receiver 不能按类型
  分派，登记 nullary_methods 表（名→func_idx，首个 impl 生效），argc==0 裸名直接 OP_CALL。compileBodies
  调序：impl/trait 方法先于 fun 体编译，使 main 体可解析。
- **有参构造器作一等值**：`val mk = Circle`（1 参 ADT 构造器）→ 合成 arity 参包装 Function
  （ctorWrapperFunc：取各参 slot + OP_MAKE_ADT），OP_CLOSURE 压栈。
- **全局作 callee**：`val mk=...; mk(x)` 裸名是全局时落通用 op_call_value（不报 Unsupported）。
- **唯一 impl 自由函数调用**：`compare(a,b)` 单 impl 时直接 OP_CALL（镜像 eval 动态类型语义，不受
  receiver 类型限制）；多 impl 退回 receiver 分派。
- **冲突 impl 防护（关键正确性）**：同一 (type_name, method_name) 多 impl → trait 组合/override 冲突
  消解（§2.7.2），VM 扁平表只取首个 → 语义不符。checkNoConflictingImpls 检出 → 整体 Unsupported 回退，
  避免静默错误输出（phase2 `to_string` 在 Ordering 上多 impl，VM 印 `Lt` vs 树遍历器 `[Lt]`）。
- 门禁：zig build test（180 VM 单测，+3 M5k）；Debug VM-on/off 33/33；ReleaseFast 33/33；30 项真跑 VM
  输出全 MATCH（仅 3 项既存并发 diff）。
- 剩余回退 3：phase2（trait override/冲突消解）、phase5（monad/HKT）、test_module_trait（多段 use +
  pack 子模块值）。

> **VM 覆盖里程碑（M5a→M5k）：9→30/33。** M5e 错误类型+char / M5f 索引赋值COW / M5g 顶层val·var /
> M5h trait声明 / M5i impl方法分派 / M5j 单段use跨文件 / M5k nullary+构造器值+冲突防护。
