    ...
    for idx in 0..node_count {
        ...
        if let Some(shl_cf) = mul_to_shl(cf) {
            ...
            for which in 0..2 {
                ...
                if std::env::var("GLUE_STRENGTH_DBG").is_ok() {
...
pub fn pass_dse(graph: &DataFlowGraph, ctx: &mut OptimizerContext) {
    ...
    for (idx, node) in graph.nodes.iter().enumerate() {
        ...
        // 死存储：加入 dead 集
        if std::env::var("GLUE_DSE_DBG").is_ok() {
...
pub fn optimize_with_analysis(
    ...
    let no_licm = std::env::var("GLUE_NO_LICM").is_ok();
    let no_unroll = std::env::var("GLUE_NO_UNROLL").is_ok();
    let no_strength = std::env::var("GLUE_NO_STRENGTH").is_ok();
    let no_dse = std::env::var("GLUE_NO_DSE").is_ok();
    // Inline pass 默认开启。GLUE_NO_INLINE=1 可显式关闭。
...
pub fn pass_inline(
    ...
    let inline_limit = std::env::var("GLUE_INLINE_LIMIT")
        .ok().and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(usize::MAX);
    if std::env::var("GLUE_INLINE_DBG").is_ok() {
...
fn inline_call(graph: &mut DataFlowGraph, ctx: &mut OptimizerContext, candidate: &InlineCandidate) {
    ...
    if let Some(eff) = effect_input {
        if eff.0 >= caller_range.0 .0 && eff.0 < caller_range.1 .0 {
            new_inputs.push(eff);
        } else if std::env::var("GLUE_INLINE_DBG").is_ok() {
