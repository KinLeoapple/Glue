...
pub fn arrayIterDeinit(obj: *ObjHeader, allocator: std.mem.Allocator) void {
    const self: *ArrayIterator = @alignCast(@fieldParentPtr("header", obj));
    self.deinit(allocator);
    allocator.destroy(self);
}

pub fn stringIterDeinit(obj: *ObjHeader, allocator: std.mem.Allocator) void {
    const self: *StringIterator = @alignCast(@fieldParentPtr("header", obj));
    self.deinit(allocator);
    allocator.destroy(self);
}

pub fn rangeIterDeinit(obj: *ObjHeader, allocator: std.mem.Allocator) void {
