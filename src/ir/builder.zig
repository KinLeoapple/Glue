...
/// 单态化类型绑定：type_param 名 → 具体 ChanType + type_id
/// 在实例化泛型函数时压栈，函数体编译期间 chanTypeFromTypeNode 查此栈
...
const BoundType = struct {
    chan_type: ChanType,
    type_id: u16, // 0 表示未知（如 ref_chan 包装的用户类型）
};
...
pub const IRBuilder = struct {
    ...
    /// 单态化类型绑定栈：实例化泛型函数时压栈，函数体编译期间 chanTypeFromTypeNode 查此栈
    /// 栈顶为当前函数的类型绑定，每项是 type_param 名 → 具体 ChanType + type_id
    type_binding_stack: std.ArrayList(TypeBinding) = .empty,
    /// 单态化缓存：键 "func_name#hash" → 特化函数索引
    monomorph_cache: std.StringHashMap(u16),
    /// 单态化进行中的函数集合（防止递归实例化无限展开）
    /// 首次进入时预占，递归命中返回预占索引
    monomorph_in_progress: std.StringHashMap(u16),
    ...
    pub fn init(allocator: std.mem.Allocator) !IRBuilder {
        ...
        var builder = IRBuilder{
            ...
            .monomorph_cache = std.StringHashMap(u16).init(allocator),
            .monomorph_in_progress = std.StringHashMap(u16).init(allocator),
        };
    ...
    pub fn deinit(self: *IRBuilder) void {
        ...
        // 释放单态化数据结构
        for (self.type_binding_stack.items) |*tb| tb.map.deinit();
        self.type_binding_stack.deinit(self.allocator);
        self.monomorph_cache.deinit();
        self.monomorph_in_progress.deinit();
        // 释放内部创建的 sema_result（单元测试场景）
        if (self.owns_sema_result) {
    ...
    pub fn build(self: *IRBuilder, module: ast.Module) BuildError!GlueIR {
        ...
        // 若 sema_result 未注入（单元测试场景），创建本地 SemaResult 并从 AST 填充类型定义
        if (self.sema_result == null) {
            const sr = try self.allocator.create(SemaResult);
            sr.* = SemaResult.init(self.allocator);
            self.sema_result = sr;
            self.owns_sema_result = true;
            try self.populateSemaResultFromAst(module);
        }
                        ...
                        switch (stmt.*) {
                            .val_decl => |vd| {
                                const chan_type = if (vd.type_annotation) |tn|
                                    ...
                                    self.inferChanTypeFromExpr(vd.value) orelse .ref_chan;
    ...
    fn allocRef(self: *IRBuilder, chan_type: ChanType) !u16 {
        return self.channels.allocRef(chan_type);
    }
    ...
    fn emitLoad(self: *IRBuilder, cell_chan: u16) BuildError!u16 {
        const meta = self.channels.get(cell_chan);
        const load_chan = try self.allocChannel(meta.chan_type);
    ...
    fn addPartialMeta(self: *IRBuilder, meta: PartialMeta) !u16 {
    ...
    fn markLastBindingAtomic(self: *IRBuilder) void {
    ...
    /// 检查类型节点是否为 Atomic<T>
    fn isAtomicType(type_node: ?*const ast.TypeNode) bool {
    ...
    fn isAtomicBinding(self: *IRBuilder, name: []const u8) bool {
    ...
    /// 从 AST 模块填充 sema_result 的类型定义和函数签名（单元测试场景：sema_result 未注入时使用）
    ///
    /// 遍历 module.declarations 中的 type_decl 和 fun_decl，为每个类型创建 TypeDefInfo
    /// 并注册，为每个函数创建 FuncSigInfo 并注册。
    /// 这使得无 sema 接入的单元测试也能通过 sema_result 查询类型/构造器/函数签名定义。
    fn populateSemaResultFromAst(self: *IRBuilder, module: ast.Module) !void {
        const arena_alloc = self.arena.allocator();
        for (module.declarations) |decl| {
            switch (decl) {
                .type_decl => |td| {
                    if (self.sema_result.?.getTypeDef(td.name) != null) continue; // 已注册（sema 接入时）
                    try self.astTypeDeclToTypeDef(td, arena_alloc);
                },
                .fun_decl => |fd| {
                    if (self.sema_result.?.getFuncSig(fd.name) != null) continue; // 已注册（sema 接入时）
                    try self.astFunDeclToFuncSig(fd, arena_alloc);
                },
                .trait_decl => |trd| {
                    if (self.sema_result.?.getTraitDef(trd.name) != null) continue; // 已注册（sema 接入时）
                    try self.astTraitDeclToTraitDef(trd, arena_alloc);
                },
                else => {},
            }
        }
    ...
    /// 将 AST fun_decl 转换为 FuncSigInfo 并注册到 sema_result（单元测试场景）
    fn astFunDeclToFuncSig(self: *IRBuilder, fd: anytype, arena_alloc: std.mem.Allocator) !void {
        ...
        for (fd.params, 0..) |p, i| {
            param_chan_types[i] = chanTypeFromTypeNode(p.type_annotation) orelse .ref_chan;
            param_is_ref[i] = if (p.type_annotation) |ta| ta.* == .ref_type else false;
        }
        const return_chan_type = chanTypeFromTypeNode(fd.return_type) orelse .i64_chan;
        const is_throwing = isThrowType(fd.return_type);
        try self.sema_result.?.putFuncSig(.{
            .name = fd.name,
    ...
    /// 将 AST trait_decl 转换为 TraitDefInfo 并注册到 sema_result（单元测试场景）
    fn astTraitDeclToTraitDef(self: *IRBuilder, trd: anytype, arena_alloc: std.mem.Allocator) !void {
        const methods = try arena_alloc.alloc(TraitMethodSig, trd.methods.len);
        for (trd.methods, 0..) |m, i| {
            const return_chan_type = chanTypeFromTypeNode(m.return_type) orelse .i64_chan;
            methods[i] = .{
                .name = m.name,
        ...
        try self.sema_result.?.putTraitDef(.{
            .name = trd.name,
            .methods = methods,
        });
    }
    ...
    fn astTypeDeclToTypeDef(self: *IRBuilder, td: anytype, arena_alloc: std.mem.Allocator) !void {
        ...
        switch (td.def) {
            .adt => |adt| {
                ...
                for (adt.constructors, 0..) |cdef, ci| {
                    ...
                    for (cdef.fields, 0..) |cf, fi| {
                        ...
                        field_chan_types[fi] = chanTypeFromTypeNode(cf.ty) orelse .ref_chan;
                ...
                try self.sema_result.?.putTypeDef(.{
                    .name = td.name,
                    .kind = .adt,
                    .constructors = ctors,
                    .type_params = type_params,
            ...
            .record => |r| {
                ...
                for (r.fields, 0..) |f, fi| {
                    ...
                    field_chan_types[fi] = chanTypeFromTypeNode(f.ty) orelse .ref_chan;
            ...
            .alias => |a| {
                try self.sema_result.?.putTypeDef(.{
                    ...
                    .target_chan_type = chanTypeFromTypeNode(a.target),
                });
            },
            .newtype => |nt| {
                ...
                field_chan_types[0] = chanTypeFromTypeNode(nt.inner) orelse .ref_chan;
            ...
            .error_newtype => |en| {
                ...
                for (en.params, 0..) |p, pi| {
                    ...
                    field_chan_types[pi] = chanTypeFromTypeNode(p.type_annotation) orelse .ref_chan;
    ...
    fn chanTypeFromTypeName(type_name: []const u8) ChanType {
        if (std.mem.eql(u8, type_name, "i8")) return .i8_chan;
        if (std.mem.eql(u8, type_name, "i16")) return .i16_chan;
        if (std.mem.eql(u8, type_name, "i32")) return .i32_chan;
        if (std.mem.eql(u8, type_name, "i64")) return .i64_chan;
        if (std.mem.eql(u8, type_name, "i128")) return .i128_chan;
        if (std.mem.eql(u8, type_name, "u8")) return .u8_chan;
        if (std.mem.eql(u8, type_name, "u16")) return .u16_chan;
        if (std.mem.eql(u8, type_name, "u32")) return .u32_chan;
        if (std.mem.eql(u8, type_name, "u64")) return .u64_chan;
        if (std.mem.eql(u8, type_name, "u128")) return .u128_chan;
        if (std.mem.eql(u8, type_name, "f16")) return .f16_chan;
        if (std.mem.eql(u8, type_name, "f32")) return .f32_chan;
        if (std.mem.eql(u8, type_name, "f64")) return .f64_chan;
        if (std.mem.eql(u8, type_name, "f128")) return .f128_chan;
        if (std.mem.eql(u8, type_name, "bool")) return .bool_chan;
        if (std.mem.eql(u8, type_name, "char")) return .char_chan;
    ...
    fn getCtorAstReturnType(self: *IRBuilder, ctor_name: []const u8) ?*ast.TypeNode {
    ...
    fn getCtorAstFieldTypeNode(self: *IRBuilder, ctor_name: []const u8, field_idx: usize) ?*ast.TypeNode {
        const mod = self.current_module orelse return null;
        for (mod.declarations) |decl| {
            switch (decl) {
                .type_decl => |td| {
    ...
    fn findTraitMethodsAst(self: *IRBuilder, trait_name: []const u8) ?[]ast.MethodDecl {
        const mod = self.current_module orelse return null;
        for (mod.declarations) |decl| {
            switch (decl) {
                .trait_decl => |trd| {
    ...
    fn findFuncParamsAst(self: *IRBuilder, name: []const u8) ?[]ast.Param {
        const mod = self.current_module orelse return null;
    ...
    fn findFuncReturnTypeAst(self: *IRBuilder, name: []const u8) ?*ast.TypeNode {
        const mod = self.current_module orelse return null;
        const dot = std.mem.indexOfScalar(u8, name, '.');
        if (dot) |idx| {
            // mangled "Type.method"：先在 type_decl 的方法中查找
    ...
    fn findFunDeclAst(self: *IRBuilder, name: []const u8) ?*const @FieldType(ast.Decl, "fun_decl") {
        const mod = self.current_module orelse return null;
        const dot = std.mem.indexOfScalar(u8, name, '.');
        if (dot) |idx| {
            // mangled "Type.method"：先在 type_decl 的方法中查找
    ...
    fn resolveFieldChanType(self: *IRBuilder, field_chan_type: ChanType, field_type_name: ?[]const u8) ChanType {
    ...
    fn getCtorTag(self: *IRBuilder, ctor_name: []const u8) ?u32 {
    ...
    fn allocParamChannels(self: *IRBuilder, params: anytype, arena_alloc: std.mem.Allocator) ![]u16 {
    ...
    fn compileFunction(self: *IRBuilder, fd: anytype, func_idx: u16) BuildError!u16 {
        ...
        for (fd.params, 0..) |param, i| {
            ...
            if (isAtomicType(param.type_annotation)) self.markLastBindingAtomic();
        }
    ...
    fn compileExpr(self: *IRBuilder, expr: *const ast.Expr) BuildError!u16 {
        ...
        switch (expr.*) {
            .int_literal => |il| {
                var sema_chan: ?ChanType = null;
                if (self.sema_result) |sr| if (sr.getExpr(@intFromPtr(expr))) |info| {
                    sema_chan = info.chan_type;
                };
    ...
    /// 编译整数字面量
    fn compileIntLiteral(self: *IRBuilder, raw: []const u8, suffix: ?[]const u8, sema_chan: ?ChanType) BuildError!u16 {
        ...
        const chan_type = sema_chan orelse blk: {
            ...
            break :blk ChanType.fromIntKind(ik);
        };
    ...
    /// 编译浮点字面量
    fn compileFloatLiteral(self: *IRBuilder, raw: []const u8, suffix: ?[]const u8, sema_chan: ?ChanType) BuildError!u16 {
        // 优先使用 Sema 推导的类型（含 expected 约束与 Rust 模式无约束回退 f32）
        const chan_type = sema_chan orelse blk: {
            const fk = floatKindFromSuffix(suffix) orelse .f32;
            break :blk ChanType.fromFloatKind(fk);
        };
    ...
    fn compileBinaryOpWithChan(self: *IRBuilder, op: ast.BinaryOp, left_chan: u16, right: *ast.Expr) BuildError!u16 {
        ...
        if (op == .eq or op == .not_eq) {
            const left_is_nullable = left_meta.chan_type == .nullable_chan;
            const right_is_nullable = right_meta.chan_type == .nullable_chan;
            const left_is_null = left_meta.chan_type == .null_chan;
            const right_is_null = right_meta.chan_type == .null_chan;
            if ((left_is_nullable and right_is_null) or (left_is_null and right_is_nullable)) {
                ...
                const is_null_out = try self.allocChannel(.bool_chan);
        ...
        if ((op == .eq or op == .not_eq) and
            left_meta.chan_type == .ref_chan and right_meta.chan_type == .ref_chan)
        {
            const eq_out = try self.allocChannel(.bool_chan);
        ...
        if (force_lazy) {
            if (left_meta.chan_type == .ref_chan and right_meta.chan_type == .ref_chan) {
                // 两侧都是惰性引用，无法从上下文推断 T：保守地强制为 i64
                left_ch = try self.emitLazyForce(left_ch, .i64_chan);
                right_ch = try self.emitLazyForce(right_ch, .i64_chan);
            } else if (left_meta.chan_type == .ref_chan) {
                left_ch = try self.emitLazyForce(left_ch, right_meta.chan_type);
            } else if (right_meta.chan_type == .ref_chan) {
                right_ch = try self.emitLazyForce(right_ch, left_meta.chan_type);
            }
            left_meta = self.channels.get(left_ch);
            right_meta = self.channels.get(right_ch);
        }
    ...
    fn emitLazyForce(self: *IRBuilder, src_chan: u16, dst_ct: ChanType) BuildError!u16 {
        const out = try self.allocChannel(dst_ct);
    ...
    fn forceLazyIfRef(self: *IRBuilder, chan: u16, expected_ct: ChanType) BuildError!u16 {
        const meta = self.channels.get(chan);
    ...
    fn forceLazyArgIfNeeded(self: *IRBuilder, arg_chan: u16, dst_chan: u16) BuildError!u16 {
        const arg_meta = self.channels.get(arg_chan);
        const dst_meta = self.channels.get(dst_chan);
        const dst_ct = dst_meta.chan_type;

        // 泛型参数装箱：标量实参 → ref_chan 形参（如 println<T>(x: T) 调用 println(42)）
        // 使用 ref_of 节点装箱标量到 BoxedScalar 堆对象，使 ref_chan 存储有效指针。
        // 运行时 builtin_str/execBuiltinStr 识别 BoxedScalar 并从通道索引读取标量值格式化。
        if (dst_ct == .ref_chan and arg_meta.chan_type != .ref_chan and arg_meta.chan_type != .nullable_chan) {
        ...
        if (arg_meta.chan_type != .ref_chan) return arg_chan;
        if (dst_ct == .ref_chan) return arg_chan;
        // nullable<T> 参数：若内部类型已是引用，直接保留引用由运行时深拷贝到 nullable 通道
        if (dst_ct == .nullable_chan and dst_meta.inner_type == .ref_chan) return arg_chan;
        if (dst_ct == .nullable_chan) return try self.emitLazyForce(arg_chan, dst_meta.inner_type);
    ...
    fn compileUnary(self: *IRBuilder, op: ast.UnaryOp, operand: *ast.Expr) BuildError!u16 {
        ...
        // 严格运算上下文：ref_chan 操作数视为 Lazy<T>，先强制求值到标量。
        if (operand_meta.chan_type == .ref_chan) {
            const force_ct: ChanType = switch (op) {
                .not => .bool_chan,
                .neg, .bit_not => .i64_chan,
            };
    ...
    fn compileRefOf(self: *IRBuilder, operand: *ast.Expr) BuildError!u16 {
        ...
        const out = try self.allocRef(.ref_chan);
    ...
    fn compileIf(self: *IRBuilder, ie: anytype) BuildError!u16 {
        ...
        cond_chan = try self.forceLazyIfRef(cond_chan, .bool_chan);
    ...
    fn compileCastBuilder(self: *IRBuilder, cb: anytype) BuildError!u16 {
        ...
        const force_ct: ChanType = if (target_is_str) .i64_chan else dst_chan_type;
        const needs_force = target_is_str or dst_chan_type.isInt() or dst_chan_type.isFloat() or dst_chan_type == .bool_chan or dst_chan_type == .char_chan;
        const raw_src_chan = try self.compileExpr(cb.expr);
        const raw_is_ref = self.channels.get(raw_src_chan).chan_type == .ref_chan;
            ...
            .call => |c| {
                if (c.callee.* == .identifier) {
                    if (self.sema_result.?.getCtorDef(c.callee.identifier.name)) |ctor| {
                        // ADT: __tag + 字段数
                        return ctor.field_chan_types.len + 1;
                    }
                }
                return 0;
            },
    ...
    fn inferFieldType(self: *IRBuilder, object: *const ast.Expr, field: []const u8) ?ChanType {
        ...
        // 不能进入下面的 .call 分支（typeof 不对应任何构造器）
        ...
        switch (obj_expr.*) {
            ...
            .call => |c| {
                // 构造器调用：Pair(1, "one") → 查 sema_result 获取字段名，按位置匹配
    ...
    /// 判断表达式是否为引用类型（&T / *T）
    fn isRefExpr(self: *IRBuilder, expr: *const ast.Expr) bool {
        if (self.sema_result) |sr| {
            if (sr.getExpr(@intFromPtr(expr))) |info| {
    ...
    fn compileIndex(self: *IRBuilder, object: *ast.Expr, index: *ast.Expr) BuildError!u16 {
        const obj_chan = try self.compileExpr(object);
        const idx_chan = try self.forceLazyIfRef(try self.compileExpr(index), .i64_chan);
    ...
    fn compileSlice(self: *IRBuilder, object: *ast.Expr, start: *ast.Expr, end: *ast.Expr, inclusive: bool) BuildError!u16 {
        const obj_chan = try self.compileExpr(object);
        const start_chan = try self.forceLazyIfRef(try self.compileExpr(start), .i64_chan);
        const end_chan = try self.forceLazyIfRef(try self.compileExpr(end), .i64_chan);
        const is_string = self.isStringExpr(object) or self.isStringParam(object);
        const out = try self.allocChannel(.ref_chan);
    ...
    fn emitStrConstant(self: *IRBuilder, s: []const u8) BuildError!u16 {
        const out = try self.allocChannel(.ref_chan);
        const str_idx = self.addString(s);
        const meta_idx = try self.addScalarMeta(.{ .kind = .str, .const_val = .{ .int_val = @intCast(str_idx) } });
        try self.emit(Node.makeSink(.const_str, out, meta_idx));
    ...
    fn emitStrConcat(self: *IRBuilder, left: u16, right: u16) BuildError!u16 {
        const out = try self.allocChannel(.ref_chan);
        const meta_idx = try self.addScalarMeta(.{ .kind = .ref });
    ...
    fn compileFieldAccessByChan(
        self: *IRBuilder,
        obj_chan: u16,
        type_name: []const u8,
        field_name: []const u8,
        field_chan_type: ChanType,
    ) BuildError!u16 {
                ...
                if (self.current_func_type_params) |tps| {
                    for (tps, 0..) |tp, idx| {
                        if (std.mem.eql(u8, tp.name, type_name)) {
                            // 单态化上下文：类型参数已绑定到具体 type_id
                            if (self.lookupTypeBinding(tp.name)) |bt| {
                                if (bt.type_id != 0) return bt.type_id;
                            }
    ...
    fn resolveReflectMetaIndex(self: *IRBuilder, arg: *const ast.Expr) BuildError!u16 {
        switch (arg.*) {
            .identifier => |id| {
                                    ...
                                    if (self.current_func_type_params) |tps| {
                                        for (tps, 0..) |tp, tp_idx| {
                                            if (std.mem.eql(u8, tp.name, nm.name)) {
                                                ...
                                                if (self.lookupTypeBinding(tp.name)) |bt| {
                                                    std.debug.print("DEBUG resolveReflectMeta: param={s}, bt.type_id={d}\n", .{ tp.name, bt.type_id });
                                                    if (bt.type_id != 0) return bt.type_id;
                                                } else {
                                                    std.debug.print("DEBUG resolveReflectMeta: param={s}, lookupTypeBinding=null, binding_stack_len={d}\n", .{ tp.name, self.type_binding_stack.items.len });
                                                }
                                                // 非单态化：发出哨兵，运行时从 frame.type_args 查表
    ...
    fn compileCallWithTypeArgs(
        ...
        // 内置函数：reflect(x) → 运行时值反射，构造 Reflect RecordValue
        // 生成 builtin_reflect 节点，meta_index = type_id 或泛型参数哨兵
        if (std.mem.eql(u8, func_name, "reflect")) {
            if (arguments.len != 1) return error.UnsupportedExpr;
            const arg_expr = arguments[0];
            const arg_chan = try self.compileExpr(arg_expr);
            const meta_idx = try self.resolveReflectMetaIndex(arg_expr);
            const out = try self.allocChannel(.ref_chan);
            try self.emit(Node.makeUnary(.builtin_reflect, out, meta_idx, arg_chan));
            return out;
        }

        // 内置函数：__scalar_to_str(x) → 标量转字符串
        if (std.mem.eql(u8, func_name, "__scalar_to_str")) {
            if (arguments.len != 1) return error.UnsupportedExpr;
            const arg_chan = try self.compileExpr(arguments[0]);
            const out = try self.allocChannel(.ref_chan);
            try self.emit(Node.makeUnary(.builtin_scalar_to_str, out, 0, arg_chan));
            return out;
        }
        ...
        std.debug.print("DEBUG compileCall: func={s}, type_args_len={d}, type_args={any}, func_idx={d}\n", .{ effective_name, type_args.len, type_args, func_idx });
        const mono_func_idx = if (type_args.len > 0)
            try self.instantiateFunction(effective_name, type_args)
        else
            func_idx;
        std.debug.print("DEBUG compileCall: func={s}, mono_func_idx={d}\n", .{ effective_name, mono_func_idx });
        ...
        for (arguments, 0..) |arg, i| {
            ...
            if (func_params) |params| {
                if (i < params.len) {
                    if (params[i].type_annotation) |tn| {
                        if (tn.* == .named) {
                            if (self.sema_result.?.getTraitDef(tn.named.name) != null) {
        ...
        if (func.is_async) {
            return try self.emitOrbitCreate(mono_func_idx, arg_chans, func);
        }
    ...
    fn inferCallTypeArgs(
        ...
        // 递归调用：从类型绑定栈继承当前函数的类型实参
        // 当泛型函数递归调用自身时（如 map(t, f) 内调用 map(t, f)），
        // 当前函数的类型参数已经在 instantiateFunction 中通过 pushTypeBinding
        // 绑定到具体 type_id。直接从栈顶读取绑定即可。
        if (self.current_func_name) |cfn| {
            if (std.mem.eql(u8, cfn, func_name) and self.type_binding_stack.items.len > 0) {
                if (self.current_func_type_params) |tps| {
        ...
        std.debug.print("DEBUG inferCallTypeArgs: func={s}, type_params_len={d}, func_params_found={}, param_count={d}\n", .{ func_name, sig.type_params.len, func_params != null, param_count });
        for (0..param_count) |i| {
        ...
        const args = try arena_alloc.alloc(u16, sig.type_params.len);
        for (sig.type_params, 0..) |tp_name, i| {
            args[i] = name_to_typeid.get(tp_name) orelse 0;
        }
        std.debug.print("DEBUG inferCallTypeArgs result: func={s}, type_params=[", .{func_name});
        for (sig.type_params) |tp| std.debug.print("{s},", .{tp});
    ...
    /// 的类型参数 T 已在 type_binding_stack 中绑定到具体 type_id。
    /// 内层调用 format(x) 时，实参 x 的类型注解仍是 T，此时从类型绑定栈
    ...
    fn matchTypeParamToTypeId(
        ...
        switch (param_type.*) {
            .named => |n| {
                ...
                // 1. 优先：实参是标识符且其类型注解是类型参数名 → 查类型绑定栈
                //    场景：println<T> 内调用 format(x)，x: T，T 已绑定到具体 type_id
                if (arg_expr.* == .identifier) {
                    if (self.lookupVar(arg_expr.identifier.name)) |binding| {
                        if (binding.type_annotation) |ta| {
                            ...
                            if (eff_ta.* == .named) {
                                if (self.lookupTypeBinding(eff_ta.named.name)) |bt| {
                                    std.debug.print("DEBUG matchTypeParam path1: param={s}, var={s}, var_type={s}, bt.type_id={d}\n", .{ n.name, arg_expr.identifier.name, eff_ta.named.name, bt.type_id });
                                    if (bt.type_id != 0) {
                                        std.debug.print("DEBUG matchTypeParam: param={s} → binding {s}={d}\n", .{ n.name, eff_ta.named.name, bt.type_id });
                                        try name_to_typeid.put(n.name, bt.type_id);
                                        return;
                                    }
                                } else {
                                    std.debug.print("DEBUG matchTypeParam path1: param={s}, var={s}, var_type={s}, lookupTypeBinding=null\n", .{ n.name, arg_expr.identifier.name, eff_ta.named.name });
                                }
                            } else {
                                std.debug.print("DEBUG matchTypeParam path1: param={s}, var={s}, eff_ta tag={s} (not named)\n", .{ n.name, arg_expr.identifier.name, @tagName(eff_ta.*) });
                            }
                        } else {
                            std.debug.print("DEBUG matchTypeParam path1: param={s}, var={s}, type_annotation=null\n", .{ n.name, arg_expr.identifier.name });
                        }
                    } else {
                        std.debug.print("DEBUG matchTypeParam path1: param={s}, var={s}, lookupVar=null\n", .{ n.name, arg_expr.identifier.name });
                    }
                }
                // 2. 从实参推导 type_id（先试类型名，再试 sema chan_type 反查）
                const tid = self.inferTypeIdFromExpr(resolved_expr);
                std.debug.print("DEBUG matchTypeParam: param={s}, expr_tag={s}, tid={d}\n", .{ n.name, @tagName(arg_expr.*), tid });
                if (tid != 0) {
                ...
                if (resolved_expr.* == .call) {
                    if (resolved_expr.call.callee.* == .identifier) {
                        ...
                        if (self.sema_result.?.getCtorDef(ctor_name)) |ctor| {
                            ...
                            //    return_type_name 只是基类名，需 AST 回退获取完整 TypeNode
                            ...
                            const field_count = @min(ctor.field_chan_types.len, ctor_args.len);
    ...
    fn inferThrowOkChanType(self: *IRBuilder, expr: *const ast.Expr) ?ChanType {
        switch (expr.*) {
            .call => |c| {
                ...
                // 普通函数调用：从 AST 查 return_type 提取 Throw Ok 通道类型
            ...
            .method_call => |mc| {
                ...
                if (self.isModuleReference(mc.object)) |mod_ref| {
                    const mangled = std.fmt.allocPrint(arena_alloc, "{s}.{s}", .{ mod_ref.full_path, mc.method }) catch return null;
                    if (self.findFuncReturnTypeAst(mangled)) |rt| {
                        return throwOkChanType(rt);
                    }
                    return null;
                }
                ...
                if (self.inferTypeNameFromExpr(mc.object)) |type_name| {
                    ...
                    if (self.lookupMethodBySuffix(type_name, mc.method)) |func_idx| {
                        if (func_idx < self.functions.items.len) {
                            ...
                            if (self.findFuncReturnTypeAst(func.name)) |rt| {
                                return throwOkChanType(rt);
                            }
                        }
                    }
    ...
    fn inferConstructorChanType(self: *IRBuilder, ctor: CtorDefInfo, arguments: []*ast.Expr) ?ChanType {
        // GADT return_type 需从 AST 回退获取完整 TypeNode
        const rt = self.getCtorAstReturnType(ctor.name) orelse return .ref_chan;
        ...
        const field_count = @min(ctor.field_chan_types.len, arguments.len);
        for (0..field_count) |i| {
            const field_type = self.getCtorAstFieldTypeNode(ctor.name, i) orelse continue;
    ...
    fn chanTypeWithTypeNode(type_node: ?*ast.TypeNode, type_bindings: std.StringHashMap(ChanType)) ?ChanType {
        ...
        return switch (tn.*) {
            ...
            .generic => |g| {
                ...
                // Atomic<T> → ref_chan（堆分配的 AtomicValue 指针）
    ...
    fn inferGenericCallReturnType(self: *IRBuilder, func_name: []const u8, arguments: []*ast.Expr) ?ChanType {
        const sig = self.sema_result.?.getFuncSig(func_name) orelse return null;
        if (sig.type_params.len == 0) return null; // 非泛型函数
        ...
        // 用绑定推导返回类型
        const func_return_type = self.findFuncReturnTypeAst(func_name) orelse return null;
        const result = chanTypeWithTypeNode(func_return_type, bindings);
        return result;
    }
    ...
    fn extractCtorTypeBinding(self: *IRBuilder, expr: *const ast.Expr, param_type: ?*ast.TypeNode, bindings: *std.StringHashMap(ChanType)) void {
        ...
        if (expr.* == .call) {
            ...
            const ctor = self.sema_result.?.getCtorDef(func_name) orelse return;
            // GADT return_type 需从 AST 回退获取完整 TypeNode
            const ctor_rt = self.getCtorAstReturnType(func_name) orelse return;

            // 如果构造器返回类型含类型参数，先从字段类型和实参推断绑定
            if (typeNodeHasTypeParam(ctor_rt)) {
                // 从字段类型和实参推断构造器自身的类型参数绑定
                const field_count = @min(ctor.field_chan_types.len, expr.call.arguments.len);
                for (0..field_count) |i| {
                    const field_type = self.getCtorAstFieldTypeNode(func_name, i) orelse continue;
                    // 递归提取子构造器的类型绑定
    ...
    fn compileConstructorCall(self: *IRBuilder, ctor: CtorDefInfo, arguments: []*ast.Expr) BuildError!u16 {
        ...
        const field_count: u32 = @intCast(ctor.field_chan_types.len + 1);
        // 计算字段引用标记位图（field_id=0 是 __tag，固定非引用；构造器字段从 1 开始）
        var field_ref_bits: u64 = 0;
        const arg_count = @min(arguments.len, ctor.field_chan_types.len);
        for (0..arg_count) |i| {
    ...
    fn addRecordMakeMeta(self: *IRBuilder, type_name: []const u8, field_count: u32, field_ref_bits: u64) !u16 {
        ...
        switch (expr.*) {
            ...
            .identifier => |id| {
                ...
                if (builder.lookupVar(id.name)) |binding| {
                    if (binding.type_annotation) |tn| {
                        switch (tn.*) {
                            .array => |a| return chanTypeFromTypeNode(a.element_type) orelse .i64_chan,
                            else => {},
                        }
                    }
    ...
    fn inferFieldArrayElemType(self: *IRBuilder, object: *const ast.Expr, field: []const u8) ?ChanType {
        // 通过 inferTypeNameFromExpr 获取对象类型名，查 sema_result 找字段 type_node
        const type_name = self.inferTypeNameFromExpr(object) orelse return null;
        const sr = self.sema_result orelse return null;
        const ctor = sr.getCtorDef(type_name) orelse return null;
        for (ctor.field_names, 0..) |fname, i| {
            if (fname) |fn_str| {
                if (std.mem.eql(u8, fn_str, field)) {
                    if (self.getCtorAstFieldTypeNode(type_name, i)) |tn| {
                        switch (tn.*) {
    ...
    fn chanTypeFromExprAst(expr: *const ast.Expr) ChanType {
        switch (expr.*) {
            .int_literal => |il| {
                // 有 suffix 用 suffix，否则 i32（Rust 模式无约束回退）
                if (il.suffix) |s| {
    ...
    fn inferTypeNameFromExpr(self: *IRBuilder, expr: *const ast.Expr) ?[]const u8 {
        if (self.sema_result) |sr| {
            ...
            if (expr.* == .identifier) {
                if (sr.getCtorDef(expr.identifier.name)) |ctor| {
                    return ctor.type_name;
                }
            }
        }
        // 回退 2：trait default 方法体中的 `self` 标识符。
        // sema 在 checkTraitDecl 中将 self 绑定为抽象 type_var，
        // semaTypeToChanType(type_var) 返回 null 导致 putExpr 未执行。
        // IRBuilder 编译继承的 default 方法时已通过 current_self_type_name
        // 记录了实现类型名（如 "Ordering"），此处直接使用。
        if (expr.* == .identifier and std.mem.eql(u8, expr.identifier.name, "self")) {
            if (self.current_self_type_name) |tn| return tn;
        }
        ...
        // 对泛型类型如 Reflect<T> 返回基类名 "Reflect"（用于方法分派）。
        if (expr.* == .identifier) {
            if (self.lookupVar(expr.identifier.name)) |binding| {
                ...
                // 无类型标注时，从初始化表达式递归推断
                // 如 val p1 = std.io.Path.path_from(...) → Path
                if (binding.ast_expr) |var_expr| {
                    return self.inferTypeNameFromExpr(var_expr);
                }
            }
        }
        // 回退 4：函数调用 / 方法调用 / 模块字段访问
        // 从被调用函数的返回类型或全局变量的类型标注推断类型名
        // 如 val p1 = std.io.Path.path_from(...) → Path
        //    val epoch = std.time.SystemTime.UNIX_EPOCH → SystemTime
        switch (expr.*) {
            .method_call => |mc| {
                if (self.isModuleReference(mc.object)) |mod_ref| {
                    const arena_alloc = self.arena.allocator();
                ...
                } else if (self.inferTypeNameFromExpr(mc.object)) |type_name| {
                    const arena_alloc = self.arena.allocator();
                    const mangled = std.fmt.allocPrint(arena_alloc, "{s}.{s}", .{ type_name, mc.method }) catch return null;
                    if (self.findFuncReturnTypeAst(mangled)) |rt| {
                        return self.typeNameFromTypeNodeSimple(rt);
                    }
                    // 短名查找失败时（如 Path.join 是 fun_decl 而非 type_decl method），
                    // 用后缀扫描定位 mangled 函数名，再用全名查返回类型
                    if (self.lookupMethodBySuffix(type_name, mc.method)) |func_idx| {
                        const full_name = self.functions.items[func_idx].name;
                        if (self.findFuncReturnTypeAst(full_name)) |rt| {
                            return self.typeNameFromTypeNodeSimple(rt);
                        }
                    }
                }
            ...
            .safe_method_call => |smc| {
                if (self.isModuleReference(smc.object)) |mod_ref| {
                    const arena_alloc = self.arena.allocator();
                    const mangled = std.fmt.allocPrint(arena_alloc, "{s}.{s}", .{ mod_ref.full_path, smc.method }) catch return null;
                    if (self.findFuncReturnTypeAst(mangled)) |rt| {
                        return self.typeNameFromTypeNodeSimple(rt);
                    }
                } else if (self.inferTypeNameFromExpr(smc.object)) |type_name| {
                    const arena_alloc = self.arena.allocator();
                    const mangled = std.fmt.allocPrint(arena_alloc, "{s}.{s}", .{ type_name, smc.method }) catch return null;
                    if (self.findFuncReturnTypeAst(mangled)) |rt| {
                        return self.typeNameFromTypeNodeSimple(rt);
                    }
                    if (self.lookupMethodBySuffix(type_name, smc.method)) |func_idx| {
                        const full_name = self.functions.items[func_idx].name;
                        if (self.findFuncReturnTypeAst(full_name)) |rt| {
                            return self.typeNameFromTypeNodeSimple(rt);
                        }
            ...
            .call => |c| {
                if (c.callee.* == .identifier) {
                    if (self.findFuncReturnTypeAst(c.callee.identifier.name)) |rt| {
                        return self.typeNameFromTypeNodeSimple(rt);
                    }
                }
            },
    ...
    fn compileReflectMethod(self: *IRBuilder, obj_chan: u16, method: []const u8, arguments: []*ast.Expr, object: *const ast.Expr) BuildError!u16 {
    ...
    fn resolveConstIndex(self: *IRBuilder, arg: *const ast.Expr) BuildError!u16 {
        // 尝试从 sema 获取编译期常量值
        if (self.sema_result) |sr| {
    ...
    fn dispatchMethodCall(self: *IRBuilder, obj_chan: u16, object: *ast.Expr, method: []const u8, arguments: []*ast.Expr) BuildError!u16 {
        ...
        if (self.isModuleReference(object)) |mod_ref| {
            ...
            if (self.func_table.get(mangled)) |func_idx| {
                ...
                // 模块引用路径（如 std.reflect.Reflect.format(x)）同样需要推断类型实参，
                // 否则泛型函数内的 reflect(x) 哨兵无法从父 frame 解析实际 type_id
                const type_args = try self.inferCallTypeArgs(mangled, arguments, null);
                // 单态化：泛型函数实例化为特化版本
                const mono_func_idx = if (type_args.len > 0)
    ...
    fn lookupMethodBySuffix(self: *IRBuilder, type_name: []const u8, method: []const u8) ?u16 {
        ...
        for (self.func_table.keys.items, self.func_table.values.items) |k, v| {
            ...
            if (self.findFuncParamsAst(k)) |params| {
                if (params.len > 0) {
                    if (params[0].type_annotation) |ta| {
    ...
    fn compileLambda(self: *IRBuilder, lam: anytype) BuildError!u16 {
        ...
        for (upvalue_chans.items, 0..) |ch, i| {
            ...
            if (meta.is_ref) upvalue_ref_bits |= @as(u8, 1) << @intCast(i);
        }
    ...
    fn compileLazyExpr(self: *IRBuilder, expr: *const ast.Expr) BuildError!u16 {
        ...
        const lazy_out = try self.allocChannel(.ref_chan);
    ...
    /// 单态化：查类型绑定栈，返回 type_param 名对应的具体 ChanType
    /// 从栈顶向下查找（内层绑定优先），未命中返回 null
    fn lookupTypeBinding(self: *IRBuilder, name: []const u8) ?BoundType {
        var i = self.type_binding_stack.items.len;
        while (i > 0) {
            i -= 1;
            if (self.type_binding_stack.items[i].map.get(name)) |bt| return bt;
        }
        return null;
    }

    /// 单态化：带类型绑定的 TypeNode → ChanType 解析
    /// 优先查 type_binding_stack，命中返回具体 ChanType；
    /// 未命中委托 chanTypeFromTypeNode（保持原行为）
    /// 递归处理 nullable/ref_type/raw_ptr/kind_annotated 的 inner
    fn chanTypeFromTypeNodeBound(self: *IRBuilder, type_node: ?*ast.TypeNode) ?ChanType {
        const tn = type_node orelse return null;
        switch (tn.*) {
            .named => |n| {
                // 1. 先查类型绑定栈（type_param 名）
                if (self.lookupTypeBinding(n.name)) |bt| return bt.chan_type;
                // 2. 未命中委托原函数（处理具体类型/内置标量/用户类型）
                return chanTypeFromTypeNode(tn);
            },
            .nullable => |nb| {
                // nullable<T>：递归解析 inner，未命中 fallback 到原函数
                return self.chanTypeFromTypeNodeBound(nb.inner) orelse chanTypeFromTypeNode(tn);
            },
            .ref_type => |rt| {
                // &T：通道存指针，固定 ref_chan；但若需 inner 类型可递归
                _ = rt;
                return .ref_chan;
            },
            .raw_ptr => |rp| {
            ...
            .kind_annotated => |ka| {
                return self.chanTypeFromTypeNodeBound(ka.inner) orelse chanTypeFromTypeNode(tn);
            },
            else => return chanTypeFromTypeNode(tn),
        }
    }

    /// 单态化：压入类型绑定（实例化泛型函数时调用）
    /// type_param 名列表与 type_args 一一对应，type_args[i] 解析为 ChanType + type_id
    fn pushTypeBinding(
        self: *IRBuilder,
        type_params: []const ast.TypeParam,
        type_args: []const u16,
    ) !void {
        var binding = TypeBinding{ .map = std.StringHashMap(BoundType).init(self.allocator) };
        errdefer binding.map.deinit();
        for (type_params, 0..) |tp, i| {
            const type_id = if (i < type_args.len) type_args[i] else 0;
            const chan_type = self.chanTypeFromTypeId(type_id);
            try binding.map.put(tp.name, .{ .chan_type = chan_type, .type_id = type_id });
        }
        try self.type_binding_stack.append(self.allocator, binding);
    }

    /// 单态化：弹出类型绑定（函数体编译结束）
    fn popTypeBinding(self: *IRBuilder) void {
        if (self.type_binding_stack.pop()) |*tb| {
            var b = tb.*;
            b.map.deinit();
        }
    }

    /// 单态化：type_id → ChanType（与 TypeMetadata 一致）
    /// type_id 0 = 未知，返回 ref_chan
    fn chanTypeFromTypeId(self: *IRBuilder, type_id: u16) ChanType {
        if (type_id == 0) return .ref_chan;
        if (type_id - 1 >= self.type_metadata_entries.items.len) return .ref_chan;
        const meta = self.type_metadata_entries.items[type_id - 1];
        // TypeMetadata.name 是具体类型名（如 "i32"/"f64"/"List"/用户类型名）
        // 内置标量名直接映射，其他返回 ref_chan
        ...
        if (std.mem.eql(u8, name, "isize")) return .isize_chan;
        if (std.mem.eql(u8, name, "usize")) return .usize_chan;
        ...
        if (std.mem.eql(u8, name, "unit")) return .unit_chan;
        // 用户自定义类型/泛型实例 → ref_chan（堆引用）
        return .ref_chan;
    }

    /// 单态化：ChanType → type_id（反查 type_name_to_id）
    /// 用于从实参的 sema chan_type 直接推导 type_id，处理 int_literal 等无 type_name 的表达式
    fn chanTypeToTypeId(self: *IRBuilder, ct: ChanType) u16 {
        const name = switch (ct) {
            .i8_chan => "i8",
            .i16_chan => "i16",
            .i32_chan => "i32",
            .i64_chan => "i64",
            ...
            .char_chan => "char",
            .unit_chan => "unit",
            else => return 0, // ref_chan(含 str/ADT)/null_chan/mask_chan/nullable_chan 等无对应 type_id
        };
    ...
    /// sema 不记录 ExprInfo（类型变量无法转 ChanType），需手动解析：
    /// 1. 从对象表达式推导基类型名 + 类型实参
    /// 2. 查类型定义获取字段的声明类型
    /// 3. 若字段类型是类型参数，用类型实参替换
    fn inferFieldAccessTypeId(self: *IRBuilder, fa: anytype) u16 {
        // 1. 解析对象的基类型名和类型实参 type_id 列表
        var base_type_name: ?[]const u8 = null;
        var type_arg_ids: std.ArrayList(u16) = .empty;
        defer type_arg_ids.deinit(self.allocator);

        // 路径 A：对象是带类型标注的变量（如 fun foo(p: Pair<i32, str>) { p.first }）
        if (fa.object.* == .identifier) {
            if (self.lookupVar(fa.object.identifier.name)) |binding| {
                if (binding.type_annotation) |ta| {
                    const effective_ta = switch (ta.*) {
                        .ref_type => |rt| rt.inner,
                        .nullable => |nb| nb.inner,
                        else => ta,
                    };
                    switch (effective_ta.*) {
                        .generic => |g| {
                            base_type_name = g.name;
                            for (g.args) |arg| {
                                const arg_name = typeNameFromTypeNodeConst(arg);
                                type_arg_ids.append(self.allocator, self.lookupTypeId(arg_name)) catch {};
                            }
                        },
                        .named => |n| base_type_name = n.name,
                        else => {},
                    }
                }
            }
        ...
        // 路径 B：对象是绑定到构造器调用的变量（如 val p1 = Pair(1, "one")）
        if (base_type_name == null) {
            if (fa.object.* == .identifier) {
                if (self.lookupVar(fa.object.identifier.name)) |binding| {
                    if (binding.ast_expr) |var_expr| {
                        if (var_expr.* == .call and var_expr.call.callee.* == .identifier) {
                            const ctor_name = var_expr.call.callee.identifier.name;
                            if (self.sema_result.?.getCtorDef(ctor_name)) |ctor| {
                                base_type_name = ctor.type_name;
                                // 从构造器调用实参推断类型实参
                                if (self.sema_result.?.getTypeDef(ctor.type_name)) |td| {
                                    if (td.constructors.len > 0) {
                                        const c = td.constructors[0];
                                        const field_count = @min(c.field_names.len, var_expr.call.arguments.len);
                                        for (0..field_count) |fi| {
                                            const arg_tid = self.inferTypeIdFromExpr(var_expr.call.arguments[fi]);
                                            type_arg_ids.append(self.allocator, arg_tid) catch {};
                                        }
                                    }
                                }
                            }
        ...
        // 路径 C：对象是构造器调用表达式（如 Pair(1, "one").first）
        if (base_type_name == null) {
            if (fa.object.* == .call and fa.object.call.callee.* == .identifier) {
                const ctor_name = fa.object.call.callee.identifier.name;
                if (self.sema_result.?.getCtorDef(ctor_name)) |ctor| {
                    base_type_name = ctor.type_name;
                    if (self.sema_result.?.getTypeDef(ctor.type_name)) |td| {
                        if (td.constructors.len > 0) {
                            const c = td.constructors[0];
                            const field_count = @min(c.field_names.len, fa.object.call.arguments.len);
                            for (0..field_count) |fi| {
                                const arg_tid = self.inferTypeIdFromExpr(fa.object.call.arguments[fi]);
                                type_arg_ids.append(self.allocator, arg_tid) catch {};
                            }
                        }
                    }
        ...
        // 路径 D：从 inferTypeNameFromExpr 获取基类型名（无类型实参）
        if (base_type_name == null) {
            base_type_name = self.inferTypeNameFromExpr(fa.object);
        }

        const type_name = base_type_name orelse return 0;
        const td = self.sema_result.?.getTypeDef(type_name) orelse return 0;

        // 2. 查找字段索引
        if (td.constructors.len == 0) return 0;
        const ctor = td.constructors[0];
        var field_idx: ?usize = null;
        for (ctor.field_names, 0..) |fn_opt, i| {
            if (fn_opt) |fn_name| {
                if (std.mem.eql(u8, fn_name, fa.field)) {
                    field_idx = i;
                    break;
                }
            }
        }
        const idx = field_idx orelse return 0;

        // 3. 从 AST 获取字段的声明类型节点
        const field_tn = self.getCtorAstFieldTypeNode(ctor.name, idx) orelse return 0;
        switch (field_tn.*) {
            .named => |n| {
                // 字段类型是具体类型或类型参数
                if (self.isTypeParamName(n.name)) {
                    // 类型参数：查 type_params 索引，用对应的 type_arg 替换
                    for (td.type_params, 0..) |tp_name, tp_idx| {
                        if (std.mem.eql(u8, tp_name, n.name)) {
                            if (tp_idx < type_arg_ids.items.len) return type_arg_ids.items[tp_idx];
                            return 0;
                        }
                    }
                    // 类型绑定栈查找（单态化上下文）
                    if (self.lookupTypeBinding(n.name)) |bt| return bt.type_id;
                    return 0;
                }
                // 具体类型名
                return self.lookupTypeId(n.name);
            },
            .generic => |g| {
                // 字段类型是泛型（如 Lst<T>）：返回基类型名对应的 type_id
                // 注意：这里不递归解析内层类型参数，仅返回基类型
                return self.lookupTypeId(g.name);
            },
            else => return 0,
        }
    }
    ...
    /// 失败时从 sema ExprInfo.chan_type 直接反查（处理 int_literal 等原始类型）
    fn inferTypeIdFromExpr(self: *IRBuilder, expr: *const ast.Expr) u16 {
        // field_access 特殊处理：sema 不记录泛型字段访问的 ExprInfo
        if (expr.* == .field_access) {
            return self.inferFieldAccessTypeId(expr.field_access);
        }
        // 1. 从类型名查
        if (self.inferTypeNameFromExpr(expr)) |type_name| {
            const tid = self.lookupTypeId(type_name);
            std.debug.print("DEBUG inferTypeIdFromExpr: type_name={s}, tid={d}\n", .{ type_name, tid });
            if (tid != 0) return tid;
        } else {
            std.debug.print("DEBUG inferTypeIdFromExpr: type_name=null, expr_tag={s}\n", .{@tagName(expr.*)});
        }
        // 2. 从 sema chan_type 反查
        if (self.sema_result) |sr| {
            if (sr.getExpr(@intFromPtr(expr))) |info| {
                // ref_chan 优先用 type_name 查（str/用户类型）
                if (info.chan_type == .ref_chan) {
                    std.debug.print("DEBUG inferTypeIdFromExpr: ref_chan type_name={?s}\n", .{info.type_name});
                    if (info.type_name) |tn| {
                        const tid = self.lookupTypeId(tn);
                        if (tid != 0) return tid;
                    }
                }
                const tid = self.chanTypeToTypeId(info.chan_type);
                if (tid != 0) return tid;
            }
        }
        return 0;
    ...
    fn hashTypeArgs(type_args: []const u16) u64 {
    ...
    fn instantiateFunction(
        ...
        std.debug.print("DEBUG instantiateFunction: func_name={s}, type_args={any}\n", .{ func_name, type_args });
        // 1. 非泛型函数：直接返回原索引
        const sig = self.sema_result.?.getFuncSig(func_name) orelse {
            std.debug.print("DEBUG instantiateFunction: no sig, fallback to func_table\n", .{});
            return self.func_table.get(func_name) orelse return error.UndefinedFunction;
        };
        if (sig.type_params.len == 0) {
            std.debug.print("DEBUG instantiateFunction: non-generic, fallback to func_table\n", .{});
            return self.func_table.get(func_name) orelse return error.UndefinedFunction;
        }
        ...
        if (self.monomorph_cache.get(cache_key)) |idx| {
            std.debug.print("DEBUG instantiateFunction: cache hit, func={s}, type_args={any}, idx={d}\n", .{ func_name, type_args, idx });
            return idx;
        }

        // 4. 查进行中（递归占位）
        if (self.monomorph_in_progress.get(cache_key)) |idx| {
            std.debug.print("DEBUG instantiateFunction: in_progress hit, func={s}, type_args={any}, idx={d}\n", .{ func_name, type_args, idx });
            return idx;
        }
        ...
        const fd = self.findFunDeclAst(func_name) orelse {
            ...
            std.debug.print("DEBUG instantiateFunction: AST not found, fallback to func_table\n", .{});
            return self.func_table.get(func_name) orelse return error.UndefinedFunction;
        };

        std.debug.print("DEBUG instantiateFunction: AST found, creating monomorphized version\n", .{});
        // 6. 预占新 Function 索引
        const new_func_idx: u16 = @intCast(self.functions.items.len);
