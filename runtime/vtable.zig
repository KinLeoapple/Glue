...
pub const TraitInfo = struct {
    ...
    /// override 方法名集合（方法名 -> void），仅 override 的方法可覆盖父 trait 实现
    override_methods: std.StringHashMap(void),
    /// 委托方法名集合（方法名 -> void），委托方法也可覆盖父 trait 实现
    delegate_methods: std.StringHashMap(void),
    /// 委托信息（方法名 -> DelegateEntry），运行时动态查找父 trait 实现
...
pub const TraitMethodEntry = struct {
    trait_name: []const u8,
    /// trait 实现的目标类型名（如类型实现 Comparable<i32> 中的 "i32"）
    type_name: []const u8,
    method_name: []const u8,
    closure: *Closure,
};

/// Trait 实现注册信息（用于关联类型查找）
pub const TraitRegistration = struct {
    trait_name: []const u8,
    type_name: []const u8,
