...
test "structural: provided superset satisfies required" {
    var mc = ModuleChecker.init(std.testing.allocator);
    defer mc.deinit();
    const provided = [_]MethodSig{
        .{ .name = "get", .arity = 1 },
        .{ .name = "put", .arity = 2 },
        .{ .name = "delete", .arity = 1 },
        .{ .name = "list", .arity = 0 },
    };
    const required = [_]MethodSig{
        .{ .name = "get", .arity = 1 },
        .{ .name = "put", .arity = 2 },
    };
    const r = mc.structurallySatisfies(&provided, &required);
    try std.testing.expect(r.ok);
}

test "structural: missing method fails" {
    var mc = ModuleChecker.init(std.testing.allocator);
    defer mc.deinit();
    const provided = [_]MethodSig{.{ .name = "get", .arity = 1 }};
    const required = [_]MethodSig{
        .{ .name = "get", .arity = 1 },
        .{ .name = "put", .arity = 2 },
    };
    const r = mc.structurallySatisfies(&provided, &required);
    try std.testing.expect(!r.ok);
    try std.testing.expect(r.reason == .missing);
    try std.testing.expectEqualStrings("put", r.missing_method.?);
}

test "structural: arity mismatch fails" {
    var mc = ModuleChecker.init(std.testing.allocator);
    defer mc.deinit();
    const provided = [_]MethodSig{.{ .name = "put", .arity = 1 }};
    const required = [_]MethodSig{.{ .name = "put", .arity = 2 }};
    const r = mc.structurallySatisfies(&provided, &required);
    try std.testing.expect(!r.ok);
    try std.testing.expect(r.reason == .arity_mismatch);
    try std.testing.expectEqual(@as(usize, 2), r.arity_expected);
    try std.testing.expectEqual(@as(usize, 1), r.arity_got);
}

test "structural: empty required always satisfied" {
    var mc = ModuleChecker.init(std.testing.allocator);
    defer mc.deinit();
    const provided = [_]MethodSig{.{ .name = "x", .arity = 0 }};
    const required = [_]MethodSig{};
    const r = mc.structurallySatisfies(&provided, &required);
    try std.testing.expect(r.ok);
}
